#!/usr/bin/env bash
# Instala o bundle PyInstaller onedir, dependências de sistema (ffmpeg, libs OpenCV/torch)
# e activa fall-detection.service.
...
APT_PACKAGES=(ffmpeg libgomp1 libglib2.0-0 libgl1 libsm6 libxext6)
...
if [[ -f "${UNINSTALL_SRC}" ]]; then
  echo "→ Instalando vigia-fall-detection-uninstall..."
  install -m 755 "${UNINSTALL_SRC}" /usr/local/bin/vigia-fall-detection-uninstall
fi
...
echo "   Remover: sudo vigia-fall-detection-uninstall"
