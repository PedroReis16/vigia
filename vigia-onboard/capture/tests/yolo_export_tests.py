...
def test_ensure_CoremlAusenteComOnnx_UsaOnnx(tmp_path: Path):
    onnx = tmp_path / "models" / "yolo" / "yolo26s-pose.onnx"
    onnx.parent.mkdir(parents=True)
    onnx.write_bytes(b"onnx-ok")

    with patch.object(ye, "is_frozen", return_value=False), patch.object(
        ye, "detect_yolo_export_backend", return_value="coreml"
    ), patch.object(ye, "artifact_matches_imgsz", return_value=True), patch.object(
        ye, "_run_ultralytics_export"
    ) as export_mock:
        result = ye.ensure_yolo_pose_export("yolo26s-pose", root=tmp_path, imgsz=320)
