...
def repo_or_bundle_root() -> Path:
    """Raiz do pacote em dev; pasta de extração do PyInstaller quando congelado."""
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        return Path(getattr(sys, "_MEIPASS"))
    # src/yolo_export.py -> parents[1] == vigia-capture/
    return Path(__file__).resolve().parents[1]


def normalize_yolo_pose_stem(model_setting: str | None) -> str:
    """Extrai o stem a partir de YOLO_MODEL (nome ou path com extensão)."""
...
def ensure_yolo_pose_export(
    ...
    *,
    ...
    # Em macOS, yolo26+ costuma falhar no CoreML — reutilizar ONNX já gerado.
    if chosen == "coreml" and backend is None:
        onnx_artifact = yolo_export_artifact_path(project_root, stem, "onnx")
        if is_valid_yolo_export_artifact(onnx_artifact, "onnx") and artifact_matches_imgsz(
            onnx_artifact, "onnx", target_imgsz
        ):
            logger.info(
                "CoreML ausente; usando export ONNX existente: %s", onnx_artifact
            )
            return onnx_artifact.resolve()
    ...
    try:
        ...
        if chosen != "coreml" or backend is not None:
            raise
        # Fallback: CoreML quebrado em alguns stems (ex.: yolo26) → ONNX.
        logger.warning(
            "Export CoreML falhou (%s); a exportar ONNX como fallback.", exc
        )
        onnx_artifact = yolo_export_artifact_path(project_root, stem, "onnx")
        return _materialize_export(stem, "onnx", onnx_artifact, target_imgsz)
